...
export const MenuButton = () => {
    ...
    return (
        <div className="md:hidden flex items-center justify-end">
            ...
            >
                <span className={`block h-[1.5px] bg-neutral-400 rounded-sm transition-all duration-300 ease-in-out ${isMenuOpen ? 'w-5.5 translate-y-1.25 rotate-45 bg-primary-300' : 'w-5.5'}`}
                />
                <span className={`block h-[1.5px] bg-neutral-400 rounded-sm transition-all duration-300 ease-in-out ${isMenuOpen ? 'opacity-0 scale-x-0' : 'w-4'}`}
                />
