            ...
            <div className="min-h-screen flex items-center mx-auto lg:px-0 xxs:py-40 lg:py-24">
                ...
                >
                    <div className="flex flex-col gap-6">
                        <span className="text-primary-400 font-display tracking-widest">Skills</span>
                        ...
                        <p className="md:w-2/3 text-neutral-400 text-lg font-body leading-relaxed text-justify">
                            Technologies I've worked with, grouped by domain. I value depth over breadth — I'd generally rather master a few tools than skim many.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                        {skills.map((stat) => (
                            <div key={stat.title} className="group rounded-xl border border-neutral-800 bg-neutral-900/20 p-5 flex flex-col gap-4 hover:border-accent-500/70 transition-colors duration-400">
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {stat.badges.map((skill: string) => {
                                        ...
                                        return (
                                            <div key={skill} className="flex flex-row gap-2 items-center bg-neutral-900/80 border border-neutral-500/30 p-1 rounded-sm">
                                                <Image
                                                    loading="lazy"
                                                    width={21}
                                                    height={21}
                                                    ...
                                                    alt={techName}
                                                    style={{ width: 21, height: 21, borderRadius: skill === 'typescript' ? 5 : 0 }}
                                                />
                                                <span className="text-sm font-body text-neutral-300">{desc ? desc : techName}</span>
                                            </div>
                                        )
                                    })}
                                </div>
